...
public class LineCHartSingleString extends View {

    private int LeftTopX = 90;      //左上 x
    private int LeftTopY = 70;      //左上 y
    private int leftBottomY = 470;  //左下 y
    private int rightBottomY = 790; //右下 y
    ...
    private ArrayList<Float> YValue = new ArrayList<>();        // y 轴坐标
    ...
    private int[] shadeColors;  //  初始化渐变色

    public LineCHartSingleString(Context context) {
        super(context);
    }

    public LineCHartSingleString(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
        initType(context, attrs);
    }

    public LineCHartSingleString(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
//        init();
    }

    private void initType(Context context, AttributeSet attrs) {
        TypedArray array = ((TypedArray) context.obtainStyledAttributes(attrs, R.styleable.LineChart));
    ...
    private void init() {
        ...
        shadeColors = new int[]{Color.parseColor("#c5dbf6"), Color.parseColor("#dae9f9"), Color.parseColor("#e3eefb")
    ...
    private void drawValue(Canvas canvas) {
        ...
        for (int i = 0; i < YValue.size() - 1; i++) {
            pathShader.moveTo(XPoint.get(i), YValue.get(i));
            pathShader.lineTo(XPoint.get(i + 1), YValue.get(i + 1));
        ...
        for (int i = 0; i < YValue.size() - 1; i++) {
            canvas.drawLine(XPoint.get(i), YValue.get(i), XPoint.get(i + 1), YValue.get(i + 1), getPaint());
        }
        // 画出转折点圆圈
        getPaint().setColor(colorLine);
        for (int i = 0; i < YValue.size(); i++) {
            ...
            canvas.drawCircle(XPoint.get(i), YValue.get(i), 14, getPaint());
            // 画中心点为白色
            getPaint().setColor(Color.WHITE);
            getPaint().setStyle(Paint.Style.FILL);
            canvas.drawCircle(XPoint.get(i), YValue.get(i), 10, getPaint());
        }
    }
    ...
    private void drawVerticalSpaceLine(Canvas canvas) {
        ...
        canvas.drawText("优", LeftTopX - 55, LeftTopY + 12, getPaint());
        canvas.drawText("良", LeftTopX - 55, LeftTopY + 112, getPaint());
        canvas.drawText("中", LeftTopX - 55, LeftTopY + 212, getPaint());
        canvas.drawText("差", LeftTopX - 55, LeftTopY + 312, getPaint());
        for (int i = 0; i < horizentalLines + 1; i++) {
            if (i == 0) {
                canvas.drawText("0", LeftTopX, leftBottomY + 50, getPaint());
//                continue;
...
//   箭头      通过路径画三角形
    ...
    public void setDates(ArrayList<String> listDate) {
        if (listDate == null || listDate.isEmpty())
            return;
        this.YValue = convertDate(listDate);
        invalidate();
    }
